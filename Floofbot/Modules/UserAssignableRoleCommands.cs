...
namespace Floofbot.Modules
{
    public class UserAssignableRoleCommands : InteractiveBase
    {
        ...
        private bool CheckRoleEntryExists(SocketRole role, SocketGuild guild)
        {
            ...
            bool isRoleInDb = new FloofDataContext().UserAssignableRoles.AsQueryable().Where(r => r.ServerId == guild.Id).Where(r => r.RoleId == role.Id).Any();
            return isRoleInDb;
        }

        [Summary("Add/Remove allowable roles for yourself")]
        public class UserAssignableRoleUserCommands : UserAssignableRoleCommands
        {
            private FloofDataContext _floofDb;
            public UserAssignableRoleUserCommands(FloofDataContext floofDb)
            {
                _floofDb = floofDb;
            }
            ...
            public async Task IAm([Summary("role name")][Remainder]string roleName = null)
            {
                if (roleName == null)
                {
                    await Context.Channel.SendMessageAsync("", false, new EmbedBuilder { Description = $"💾 Usage: `iam [rolename]`", Color = base.GenerateColor() }.Build());
                    return;
                }
        ...
        [Group("iamconfig")]
        public class UserAssignableRoleAdminCommands : UserAssignableRoleCommands
        {
            ...
            public async Task AddRole([Summary("role name")][Remainder]string roleName = null)
            {
                if (roleName == null)
                {
                    await Context.Channel.SendMessageAsync("", false, new EmbedBuilder { Description = $"💾 Usage: `iamconfig add [rolename]`", Color = GenerateColor() }.Build());
                    return;
                }
            ...
            public async Task RemoveRole([Summary("role name")][Remainder]string roleName = null)
            {
                if (roleName == null)
                {
                    await Context.Channel.SendMessageAsync("", false, new EmbedBuilder { Description = $"💾 Usage: `iamconfig remove [rolename]`", Color = GenerateColor() }.Build());
                    return;
                }
