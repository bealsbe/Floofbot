            ...
            if (alertMessageIdsDic != null && alertMessageIdsDic.ContainsKey(msg.Id))
            {
                ...
                if (reaction.Emote.Name.Equals(BAN_EMOJI.Name))
                {
                    try
                    {
                        ...
                        await channel.Value.SendMessageAsync($"Got it! I banned {badUser.Username}#{badUser.Discriminator}!");
                    }
                    catch (Exception ex)
                    {
                        await channel.Value.SendMessageAsync("Unable to ban user. Do I have the permissions?");
                ...
                else if (reaction.Emote.Name.Equals(WARN_EMOJI.Name))
                {
                    try
                    {
                        ...
                        await channel.Value.SendMessageAsync($"Got it! I warned {badUser.Username}#{badUser.Discriminator}!");
                    }
                    catch (Exception ex)
                    {
                        await channel.Value.SendMessageAsync("Unable to warn user. Do I have the permissions?");
                ...
                else if (reaction.Emote.Name.Equals(KICK_EMOJI.Name))
                {
                    try
                    {
                        ...
                        await channel.Value.SendMessageAsync($"Got it! I kicked {badUser.Username}#{badUser.Discriminator}!");
                    }
                    catch (Exception ex)
                    {
                        await channel.Value.SendMessageAsync("Unable to kick user. Do I have the permissions?");
                ...
                else if (reaction.Emote.Name.Equals(REMOVE_NICKNAME_EMOJI.Name))
                {
                    try
                    {
                        ...
                        await channel.Value.SendMessageAsync($"Got it! I removed {badUser.Username}#{badUser.Discriminator}'s nickname!");
                    }
                    catch (Exception ex)
                    {
                        await channel.Value.SendMessageAsync("Unable to remove their nickname. Do I have the permissions?");
                ...
                {
                    await channel.Value.SendMessageAsync($"Got it! I took no action against {badUser.Username}#{badUser.Discriminator}!");
                    alertMessageIdsDic.Remove(msg.Id);
                }
                return;
            }
